...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function checkcasename(a){
			if(a.length>0){
				$("#checkname_error").html("");
				aa=true;
				$.post("case/checkname.do",{
					"casename":a
				},function(data){
					if(data=="Y"||data=="\"Y\""){
						bb=true;
					}else{
						$("#checkname_error").html("<font color=red>该用例名称已存在！</font>");
						bb=false;
					}
				});
			}else{
				$("#checkname_error").html("<font color=red>请填写用例名称！</font>");
				aa=false;
			}
		}
	</script>
  ...
  <form action="case/addcase.do" method="post">
	<table class="tck" width="213" border="0" cellspacing="0" cellpadding="0">
	   <tr bgcolor=white><td align=right>系&nbsp;&nbsp;统：</td><td>
		...
		<input class="text_yhm" name="casename" onblur="checkcasename(this.value)" maxlength="32"/><span id="checkname_error"></span>
		</td></tr>
		...
		<tr bgcolor=white><td class="tc_w108" align=right>类型：</td><td>
		  <select name="type">
			 ...
			 <option value="postjson">json</option>
		</select>
		</td></tr>
