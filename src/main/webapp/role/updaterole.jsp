...
<html>
  ...
  <form action="/role/updaterole.do" method="post">
	...
	<table class="tck" width="213" border="0" cellspacing="0" cellpadding="0">
		...
		<input class="text_yhm" name="rolename" value="${data.role.rolename }" id="rolename" onblur="check_rolename(this.value)"/><span id="checkname_error"></span>
		</td></tr>
		<tr bgcolor=white><td align=right class="tc_w108">描&nbsp;&nbsp;述：</td><td>
		<input class="text_yhm" name="description" value="${data.role.description }" onblur="check_description(this.value)"/><span id="description_error"></span>
		</td></tr>
		<tr bgcolor=white><td align=right>权&nbsp;&nbsp;限：</td><td>
			<c:forEach items="${data.functions}" var="o">
				<c:if test="${o.parentid=='0'}">
					<input type="checkbox" id="${o.funcid }" name="aaa" value="${o.funcid }"
							<c:forEach items="${data.funcRoles}" var="f">
								<c:if test="${f.funcid eq o.funcid}">checked</c:if>
							</c:forEach>
					/>
					<b>${o.funcname }</b><br>
					<c:forEach items="${data.functions}" var="e">
						<c:if test="${e.parentid eq o.funcid && (e.url==null || e.url=='')}">
							&nbsp;<input type="checkbox" id="${e.funcid }" name="aaa" value="${e.funcid }" onclick="check_parentid('${e.parentid }')"
							<c:forEach items="${data.funcRoles}" var="f">
								<c:if test="${f.funcid==e.funcid}">checked</c:if>
							</c:forEach> />
							<b>${e.funcname }</b><br>
							<c:forEach items="${data.functions}" var="a">
								<c:if test="${a.parentid eq e.funcid && (a.url==null || a.url=='')}">
									&nbsp;<input type="checkbox" id="${a.funcid }" name="aaa" value="${a.funcid }" onclick="check_parentid('${a.parentid }')"
									<c:forEach items="${data.funcRoles}" var="f">
								...
								<c:if test="${a.parentid eq e.funcid && a.url!=null && a.url!=''}">
									...
									<c:forEach items="${data.functions}" var="f">
										<c:if test="${f.funcid eq a.funcid}">checked</c:if>
									</c:forEach> />
									${a.funcname }<br>
								</c:if>
